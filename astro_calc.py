...
def _calc_positions(jd: float, lat: float, lon: float) -> dict:
    ...
    if nn:
        ...
        positions["Nœud Sud ☋"] = {
            "lon": ks_lon, "speed": 0, "retrograde": False,
    ...
    if moon:
        ...
        positions["ASC ↑"] = {
            "lon": moon_sign_start, "speed": 0, "retrograde": False,
    ...
    try:
        ...
        mc_lon = ascmc[1] % 360
        positions["MC ↑"] = {
            "lon": mc_lon, "speed": 0, "retrograde": False,
...
def calculate_transits(natal: dict, transit_loc: dict,
    ...
    # ── Données d'affichage — lon_raw ajouté pour la Carte Karmique SVG ──────
    def _display_dict(pos_dict):
        return {
            k: {
                "display":   v["display"],
                "retrograde": v["retrograde"],
                "lon_raw":   round(v["lon"], 4),   # ← nécessaire pour la roue SVG
            }
